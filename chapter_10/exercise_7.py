# Modify the PDB file reader of Section 10.7, Multiline Records, on page 197,
# so that it ignores blank lines and comment lines in PDB files. A blank
# line is one that contains only space and tab characters (that is, one that
# looks empty when viewed). A comment is any line beginning with the
# keyword CMNT.
...
def read_molecule(reader):
    ...
    while not line.startswith('END') or line.strip() == '':
